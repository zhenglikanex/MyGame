...
public class GameStartState : FSMState {
    public override void Enter()
    {
        Debug.Log("游戏启动");
        // 初始化CPlusPlus
        CPlusPlusBridge.SetUnityDelegate(CPlusPlusBridge.UnityCallback);

        UIManager.Instance.Initalized();
        UIManager.Instance.SetUIPath("Prefab/UI/");

        //加载moudle
        MoudleManager.Instance.Initalized();

        // 切换到登入状态
        GameController.Instance.DoTransition(Transition.kGameToBattleTrans);
