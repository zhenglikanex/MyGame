...
ClientNetworkImpl::ClientNetworkImpl()
	: running_(false)
...
bool ClientNetworkImpl::Connect(const std::string& ip, uint16_t port, uint32_t timeout)
{
...
void ClientNetworkImpl::Disconnect()
{
...
bool ClientNetworkImpl::IsConnected() const
{
	return type_ == ConnectType::kTypeConnected;
}

void ClientNetworkImpl::Send(std::vector<uint8_t>&& data)
{
...
void ClientNetworkImpl::Send(uint8_t* data, uint32_t len)
{
...
bool ClientNetworkImpl::IsEmpty() const
{
	return cur_read_ == cur_write_;
}

std::unique_ptr<std::vector<uint8_t>> ClientNetworkImpl::PopBuffer()
{
	if (IsConnected())
	{
		if (!IsEmpty())
...
int ClientNetworkImpl::UdpOutput(const char* buf, int len, ikcpcb* kcp, void* user)
{
	auto service = (ClientNetworkImpl*)user;
	service->UdpSend((uint8_t*)buf, len);
	return 0;
}

void ClientNetworkImpl::Connecting(uint32_t timeout)
{
...
void ClientNetworkImpl::Connected(kcp_conv_t conv)
{
	connect_timer_.cancel();
	...
	kcp_->output = &ClientNetworkImpl::UdpOutput;
...
void ClientNetworkImpl::Timeout()
{
	type_ = ConnectType::kTypeDisconnect;
	connect_timer_.cancel();
}

void ClientNetworkImpl::SendConnectedMsg()
{
	// 发送连接上的消息
...
void ClientNetworkImpl::UdpSend(uint8_t* data, uint32_t len)
{
...
void ClientNetworkImpl::UdpReceive()
{
...
void ClientNetworkImpl::KcpReceive(uint8_t* data, uint32_t len)
{
...
void ClientNetworkImpl::KcpUpdate()
{
	if (kcp_ && IsConnected())
	{
