...
#define SERIALIZE_MAKE_HAS_MEMEBER_TEST(name)																						\
	...
	{																																\
		...
		static auto test(int) -> decltype(Access::name(std::declval<AA&>(),std::declval<std::remove_cv_t<std::remove_reference_t<TT>>&>()), std::true_type());					\
																																	\
...
#define SERIALIZE_MAKE_HAS_NON_MEMBER_TEST(name)																	\
	...
	struct is_output_serializable<Archive,T> :
		std::integral_constant<bool,has_member_Serialize<T,Archive>::value || has_non_member_Serialize<T,Archive>::value || has_member_Save<T,Archive>::value || has_non_member_Save<T,Archive>::value>{};
	...
	struct is_input_serializable<Archive, T> :
		std::integral_constant<bool, has_member_Serialize<T, Archive>::value || has_non_member_Serialize<T, Archive>::value || has_member_Load<T, Archive>::value || has_non_member_Load<T, Archive>::value> {};
}
