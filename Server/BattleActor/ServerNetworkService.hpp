...
{
public:
	ServerNetworkService(const std::function<std::unique_ptr<std::tuple<std::string, std::vector<uint8_t>>>()>& recv_handler,
...
private:
	std::function<std::unique_ptr<std::tuple<std::string, std::vector<uint8_t>>>()> recv_handler_;
